...
import ejb.ClienteJuridicoEntidadFacade;
 ...
 * @author lily
 */
@WebServlet(name = "ListClienteJuridico", urlPatterns = {"/ListClienteJuridico"})
public class ListClienteJuridico extends HttpServlet {

    @EJB
    private EjbSessionManagerBean ejbSessionManagerBean;

    @EJB
    private ClienteJuridicoEntidadFacade ClienteJuridicoEntidadFacade;
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<th class='column1'>Razon Social</th>");
            out.println("<th class='column1'>RUC</th>");
            out.println("<th class='column1'>Correo</th>");
            out.println("<th class='column1'>Telefono</th>");
            out.println("<th class='column1'>Fecha</th>"); 
            out.println("</tr>");
            out.println("</thead>");

            out.println("<tbody>");
            out.println("<tr>");
            List emp=ClienteJuridicoEntidadFacade.findAll();
            for (Iterator it = emp.iterator(); it.hasNext();){
                    ClienteJuridicoEntidad elem= (ClienteJuridicoEntidad) it.next();           
                    out.println("<td class='column1'>"+elem.getRazon_social()+"</td>");
                    out.println("<td class='column1'>"+elem.getRuc()+"</td>");
                    out.println("<td class='column1'>"+elem.getCorreo()+"</td>");  
                    out.println("<td class='column1'>"+elem.getTelefono()+"</td>");
                    Calendar cal =elem.getFecha();
                    Date fechaIn = cal.getTime();
                    DateFormat formatoFecha=DateFormat.getDateInstance(java.text.DateFormat.FULL);
                    out.println("<td class='column1'>"+formatoFecha.format(fechaIn)+"</td>"); 
                    out.println("</tr>");
            }
            out.println("</tbody>");
            out.println("</table>");
